          ...
          <Link key={item.title} href={item.href} className="rounded-4xl">
            <Card className="h-full max-w-xs rounded-4xl p-4 sm:max-w-md sm:p-8">
              <CardContent className="flex flex-1 flex-col px-0">
                    ...
                    <div className="text-muted-foreground mt-2 flex items-center gap-2">
                      {item.badges.map((badge) => (
                        <Badge
                          variant="secondary"
                          key={badge.title}
                          className="bg-cc-sage-100 max-sm:text-2xs dark:bg-cc-sage-900 gap-1 rounded-md px-2 py-1 md:gap-2 md:px-2.5 md:py-3.5 md:[&>svg]:size-4!"
                        >
                          <HugeiconsIcon
                            icon={badge.icon}
                            strokeWidth={1.75}
                ...
                <Button
                  variant="ghost"
                  className="mt-auto mr-auto px-0 hover:bg-transparent max-sm:text-xs"
                >
                  {item.buttonLabel}
